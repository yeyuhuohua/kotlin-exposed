...
- 审计日志（仅 ADMIN）：登录尝试（成功与失败）记录用户名、IP、时间与失败原因；`/api` 调用记录用户、方法、路径、状态码与耗时。两张表只增不改，记录只增不减，后续可按需加定期清理。
...
登录页的“记住密码”是可选功能：勾选后凭据会写入 `localStorage`（密码经 Base64 编码，仅避免明文直接展示，不是加密），用于下次自动填充；公共设备请勿使用，取消勾选或在未勾选状态下登录会立即清除。
